...
function ReportsPanel({items, qaStore, conn, onSelect}) {
  ...
  if (view==="aging") return <div style={{height:"100%",display:"grid",gridTemplateRows:"40px 1fr",gridTemplateColumns:"230px 1fr",background:dashBg,color:dashText,overflow:"hidden"}}>
    <div style={{gridColumn:"1 / -1",height:40,background:"#05642a",display:"flex",alignItems:"center",gap:12,padding:"0 14px",fontWeight:800}}>
      <button aria-label="Menu" style={{width:24,height:24,border:"none",background:"transparent",color:"#fff",fontSize:18,cursor:"pointer"}}>☰</button>
      <span style={{fontSize:16}}>Nucor - ADO Dashboard</span>
      <span style={{marginLeft:"auto",fontSize:12,fontWeight:600,opacity:.95}}>{conn?.org || "Nucor"} / {conn?.projectName || "Project"}</span>
    </div>
    <nav style={{gridColumn:1,gridRow:2,background:"#2f363c",borderRight:"1px solid #1f252b",display:"flex",flexDirection:"column"}}>
      {[["aging","Testing Aging Dashboard",Ic.FileText],["all","ADO Bug Dashboard",Ic.Factory],["release","Feature Delivery Dashboard",Ic.BarChart],["plan","Report Dashboard",Ic.FileText],["complete","Feature Completion Dashboard",Ic.Check]].map(([id,label,Icon])=><button key={id} onClick={()=>setView(id)} style={{height:45,display:"flex",alignItems:"center",gap:12,padding:"0 14px",border:"none",borderBottom:"1px solid #454c52",background:view===id?"#3b4248":"transparent",color:"#fff",fontSize:13,fontWeight:view===id?800:600,textAlign:"left",cursor:"pointer"}}><Icon size={15}/><span style={{overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{label}</span></button>)}
    </nav>
    <main style={{gridColumn:2,gridRow:2,overflow:"auto",background:"#0b0f14"}}>
      <section style={{background:"#15191f",borderBottom:`1px solid ${dashLine}`,padding:"16px 16px 32px"}}>
        <div style={{fontSize:15,fontWeight:900,marginBottom:18}}>QA Testing KPIs</div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(5,minmax(0,1fr))",border:`1px solid ${dashLine}`,borderRadius:7,overflow:"hidden"}}>
          {[
          ...
          ].map(([label,value,color])=><div key={label} style={{padding:"18px 20px",borderRight:`1px solid ${dashLine}`,background:"#15191f"}}><div style={{fontSize:28,fontWeight:800,color}}>{value}</div><div style={{fontSize:10,textTransform:"uppercase",letterSpacing:"0.04em",color:dashMuted,marginTop:6}}>{label}</div></div>)}
        </div>
      </section>
      <section style={{maxWidth:1370,margin:"0 auto",padding:"34px 18px",display:"flex",flexDirection:"column",gap:12}}>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          <div><div style={{fontSize:22,fontWeight:900}}>ADO Testing Aging Report</div><div style={{fontSize:11,color:dashMuted}}>{conn?.projectName} · {conn?.org} · As of {new Date().toLocaleDateString()}</div></div>
        ...
        <details open style={{border:`1px solid ${dashLine}`,borderRadius:7,background:dashCard,overflow:"hidden"}}>
          <summary style={{padding:"10px 14px",fontSize:13,fontWeight:800,cursor:"pointer",listStyle:"none"}}>Requirements ({dashRows.length}) <span style={{float:"right"}}>▼</span></summary>
          <div style={{borderTop:`1px solid ${dashLine}`,padding:12,display:"flex",gap:8,alignItems:"end",flexWrap:"wrap"}}>
            <DashSelect label="State" value={dashFilters.state} options={dashOptions.states} onChange={v=>setDashFilters(f=>({...f,state:v}))}/><DashSelect label="Type" value={dashFilters.type} options={dashOptions.types} onChange={v=>setDashFilters(f=>({...f,type:v}))}/><DashSelect label="Priority" value={dashFilters.priority} options={dashOptions.priorities} onChange={v=>setDashFilters(f=>({...f,priority:v}))}/><DashSelect label="Owner" value={dashFilters.owner} options={dashOptions.owners} onChange={v=>setDashFilters(f=>({...f,owner:v}))}/><DashSelect label="Release" value={dashFilters.release} options={dashOptions.releases} onChange={v=>setDashFilters(f=>({...f,release:v}))}/>
            <label style={{display:"flex",flexDirection:"column",gap:5,minWidth:180}}><span style={{fontSize:10,color:dashMuted,fontWeight:800,textTransform:"uppercase"}}>Search</span><input value={dashFilters.search} onChange={e=>setDashFilters(f=>({...f,search:e.target.value}))} placeholder="ID or title..." style={{height:30,border:`1px solid ${dashLine}`,borderRadius:5,background:"#1d232d",color:dashText,fontSize:12,padding:"0 8px"}}/></label>
          </div>
        </details>
        <div style={{border:`1px solid ${dashLine}`,borderRadius:7,background:dashCard,padding:10,display:"flex",alignItems:"center",gap:12}}><span style={{fontSize:12,color:dashMuted}}>Days:</span><b>0</b><input type="range" min="0" max="1000" value={dashFilters.maxDays} onChange={e=>setDashFilters(f=>({...f,maxDays:e.target.value}))} style={{width:220}}/><b>{dashFilters.maxDays}</b><Btn size="sm" variant="ghost" onClick={()=>setDashFilters({state:"",type:"",priority:"",owner:"",release:"",search:"",maxDays:1000})}><Ic.Close size={12}/> Reset</Btn><Btn size="sm" variant="ghost" onClick={exportAgingCSV}><Ic.Download size={12}/> Export XLSX</Btn></div>
        {groupedDash.map(([priority,rows])=><div key={priority}><div style={{fontSize:16,fontWeight:900,margin:"14px 0 6px"}}>{priorityName(priority)} <span style={{fontSize:11,color:dashMuted,background:"#263241",borderRadius:9,padding:"2px 8px",marginLeft:6}}>{rows.length}</span></div><table style={{width:"100%",borderCollapse:"collapse",fontSize:12,background:dashCard,border:`1px solid ${dashLine}`,borderRadius:7,overflow:"hidden"}}><thead><tr>{["ID","Type","Description","State","Priority","Days","Owner","Planned Complete","Project","Solution Build"].map(h=><th key={h} style={{textAlign:"left",padding:"9px 10px",borderBottom:`1px solid ${dashLine}`,color:dashMuted,fontSize:10,textTransform:"uppercase"}}>{h} ⇅</th>)}</tr></thead><tbody>{rows.map(item=><tr key={item.id} onClick={()=>onSelect?.(item.id)} style={{cursor:"pointer"}}><td style={{padding:"9px 10px",borderBottom:`1px solid ${dashLine}`,color:"#60a5fa",fontWeight:800}}>{item.id}</td><td style={{padding:"9px 10px",borderBottom:`1px solid ${dashLine}`}}><span style={{padding:"2px 7px",borderRadius:5,background:/bug/i.test(item.type)?T.red:T.violet,color:"#fff",fontSize:10,fontWeight:800}}>{item.type}</span></td><td style={{padding:"9px 10px",borderBottom:`1px solid ${dashLine}`,fontWeight:700}}>{item.title}</td><td style={{padding:"9px 10px",borderBottom:`1px solid ${dashLine}`}}><span style={{padding:"2px 8px",borderRadius:9,background:isQAState(item)?"#0b2b63":"#2d1858",color:isQAState(item)?"#60a5fa":"#c084fc",fontSize:11,fontWeight:800}}>{item.state}</span></td><td style={{padding:"9px 10px",borderBottom:`1px solid ${dashLine}`,color:T.red,fontWeight:900}}>{priority==="Critical"?"P1":priority}</td><td style={{padding:"9px 10px",borderBottom:`1px solid ${dashLine}`}}><span style={{padding:"2px 8px",borderRadius:9,background:pendingDays(item)>14?"#4c1d1d":"#374151",color:pendingDays(item)>14?T.red:dashText,fontWeight:800}}>{pendingDays(item)}d</span></td><td style={{padding:"9px 10px",borderBottom:`1px solid ${dashLine}`}}>{item.assignedTo||"Unassigned"}</td><td style={{padding:"9px 10px",borderBottom:`1px solid ${dashLine}`,color:dashMuted}}>—</td><td style={{padding:"9px 10px",borderBottom:`1px solid ${dashLine}`,color:"#60a5fa"}}>{conn?.projectName}</td><td style={{padding:"9px 10px",borderBottom:`1px solid ${dashLine}`}}>{(item.areaPath||"").split("\\").pop()}</td></tr>)}</tbody></table></div>)}
      </section>
    </main>
  </div>;
