...
function findSpec(grade, standard = '') {
...
function validateTraceability(args) {
...
server.setRequestHandler(CallToolRequestSchema, async (req) => {
  ...
  try {
    ...
    if (name === 'spec_lookup') {
      ...
      out = { spec: hit.name, ...hit.spec, caution: 'Demo reference only. Confirm against the current standard edition/customer specification before certification.' };
    } else if (name === 'spec_evaluate') {
      out = evaluate(args);
    } else if (name === 'validate_traceability') {
      out = validateTraceability(args);
    } else {
